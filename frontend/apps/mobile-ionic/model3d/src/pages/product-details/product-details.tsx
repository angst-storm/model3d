            ...
            <div className={`${styles['product-details']}`}>
                {
                    productRequestState.currentData && !productRequestState.isLoading && <>
                        <div className={`${styles['characteristics-block']}`}>
                            <div className={`${styles['title-row']}`}>
                                <h1 className={`${styles['title']} M3-headline-s`}>{productRequestState.currentData.name}</h1>
                                <img src={require('@assets/icons/svg/heart.svg').default}/>
                                <span
                                    className={`${styles['purchase-count']} M3-title-m`}>{productRequestState.currentData.purchaseCount}</span>
                            </div>
                            <div className={`${styles['image-block']}`}>
                                {
                                    !model && <img className={`${styles['active-image']}`}
                                                   src={productRequestState.currentData.image}/>
                                }
                                {
                                ...
                                <ul className={`${styles['characteristics-list']}`}>
                                    {
                                        characteristicsList.map((characteristic) => {
                                            return <li
                                                className={`${styles['characteristics-element']}`}>{characteristic}</li>
                                        })
                                    }
                                </ul>
                            </div>
                            ...
                            }
                            {
                                ...
                                <span className={`${styles['price']} M3-headline-s`}>
                                    {productRequestState.currentData.cost} ₽
                                        ...
                                        <div className={`${styles['price-label']} M3-label-s`}
                                             data-free={productRequestState.currentData.isFree}>
